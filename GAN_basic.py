...
"""
# TODO all data should be scaled in all model
...
class GAN_basic(object):
    ...
    def build(self):
        ...
        disc_real = self.discriminator(self.x)
        disc_fake = self.discriminator(self.G_z)
    ...
    def fit(self, X_in, path, num_gen=100, num_epoch=30):
        ...
        for epoch in range(num_epoch):
            ...
            for iter in range(X_in.shape[0] // self.batch_size):
                ...
                loss_d_, _ = self.sess.run([self.D_loss, self.D_optim], {self.x: x_, self.z: z_})
    ...
    def get_generation(self, z):
        return self.sess.run(self.G_z, feed_dict={self.z: z})
...
def main(noise_factors, debug=True):
    ...
    for noise_factor in noise_factors:
        ...
        dcgan = GAN_basic(sess, noise_dim=hidden_dim, learning_rate=1e-3, batch_size=batch_size)
