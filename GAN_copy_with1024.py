...
def generator(x):
    ...
    # 1st hidden layer
    w6 = tf.get_variable('G_w6', [h0.get_shape()[1], 384], initializer=w_init)
    b6 = tf.get_variable('G_b6', [384], initializer=b_init)
    h6 = tf.nn.relu(tf.matmul(h0, w6) + b6)

    # 2nd hidden layer
    w1 = tf.get_variable('G_w1', [h6.get_shape()[1], 512], initializer=w_init)
    b1 = tf.get_variable('G_b1', [512], initializer=b_init)
    h1 = tf.nn.relu(tf.matmul(h6, w1) + b1)
...
def discriminator(x, drop_out):
    ...
    # 2nd hidden layer
    w6 = tf.get_variable('D_w6', [h1.get_shape()[1], 384], initializer=w_init)
    b6 = tf.get_variable('D_b6', [384], initializer=b_init)
    h6 = tf.nn.relu(tf.matmul(h1, w6) + b6)
    h6 = tf.nn.dropout(h6, drop_out)

    # 3rd hidden layer
    w2 = tf.get_variable('D_w2', [h6.get_shape()[1], 256], initializer=w_init)
    b2 = tf.get_variable('D_b2', [256], initializer=b_init)
    h2 = tf.nn.relu(tf.matmul(h6, w2) + b2)
...
digit_n = 10
fixed_z_ = np.random.normal(0, 1, (digit_n*digit_n, 100))
def show_result(num_epoch, show = False, save = False, path = 'result.png', isFix=False):
    z_ = np.random.normal(0, 1, (digit_n*digit_n, 100))
