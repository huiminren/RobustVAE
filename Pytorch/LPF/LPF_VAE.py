...
#        noise_factors = [round(0.01*i,2) for i in range(1,52,4)]
    ...
    for noise_factor in noise_factors:
        x_train_noisy = random_noise(image = x_train, amount = noise_factor).astype(np.float32)# change type to float32
        
        # remove noise by LPF
        x_train_denoised = []
        for data in x_train_noisy:
            x_train_denoised.append(cv2.blur(data,(3,3)))
        x_train_denoised = np.array(np.transpose(x_train_denoised,(0,3,1,2))) # change 2nd dimension as channel
#        x_train_noisy = np.transpose(x_train_noisy,(0,3,1,2)) 
        
        train_dataset = Data.TensorDataset(torch.tensor(x_train_denoised))
        ...
        recon = vae.reconstruction(device=device, model=model, dataloader=recon_loader)
        
        # get generation
        vae.generation_fid(device=device, model=model, path=root)
