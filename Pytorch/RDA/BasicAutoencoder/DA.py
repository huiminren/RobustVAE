...
class AE_Net(nn.Module):
    ...
    def encoder(self,x):
        ...
        return self.fc1(h4) # in_8192, out_nz
    ...
    def forward(self, x):
        z = self.encoder(x.view(-1, self.nc, self.ndf, self.ngf))
        res = self.decoder(z)
        return res
...
#        noise_factors = [round(0.01*i,2) for i in range(1,52,4)]
    ...
    for noise_factor in noise_factors:
        ...
        # get reconstruction
        recon_loader = Data.DataLoader(train_dataset, batch_size=128, shuffle=False,num_workers=2)
