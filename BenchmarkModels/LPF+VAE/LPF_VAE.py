...
def main(data_source, noise_method, noise_factors,debug=True):
    ...
    for noise_factor in noise_factors:
        ...
        x_train_clean = multi_image(x_train_noisy,step=1,runs=2)
        ...
        with tf.Session() as sess:
            vae = VariantionalAutoencoder(sess = sess, input_dim = x_train_clean.shape[1], n_z=n_z)
            vae.fit(x_train_clean, path = path,file_name="loss", 
